...
module Jekyll
    ...
    #  items:                 # items in the topic; each has three fields
    #  - url: http://.../     # item's URL
    ...
    class TableFromFile < Liquid::Tag
        ...
        def initialize(tag_name, markup, tokens)
            ...
            if markup =~ SYNTAX
                ...
                raise SyntaxError.new("Syntax error in '#{@directive}': " +
                                      "Must be #{name} yaml_file [options]")
            end
        end
        ...
        def render(context)
            ...
            if not File.exists?(yaml_file)
                return "Can't find file #{@yaml} in \\_includes directory."
            end
            ...
            if not data.is_a? Array
                return "#{@yaml}: Expected sequence of sections."
            end
